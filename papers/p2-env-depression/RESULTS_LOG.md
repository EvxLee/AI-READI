...
| EG.33 | done | `results/EG_33_fdr_corrected.csv` | Project-head-requested (2026-09-03, via user "run them both"): Benjamini-Hochberg FDR correction on the combined 42 env-SDOH tests (EG.27's 36 + EG.32's 6 new food-insecurity tests) — closes the multiple-comparisons gap both flagged as uncorrected. Track: primary. | **25/42 tests survive FDR at q<0.05** (26/42 were raw-significant; only 1 drops out, mean_voc/paid_total). All 6 food-insecurity pairs survive, PM2.5/food_insecurity_score strongest by far (q=4.9e-34). This is NOT an artifact of testing 42 things at once — the env-SDOH signal is mostly real by FDR standards, unlike EG.18's pollution-grid where correction cut 44 raw hits down to 17. | keep — confirms most of EG.27/EG.32's raw hits are statistically real, sets up EG.34's severity check on the 25 survivors |
| EG.34 | done | `results/EG_34_summary.csv` | Project-head-requested (2026-09-03, via user "run them both"): severity-adjusted OLS refit (sdoh_score ~ env_metric + age + site + severity-group dummies) of all 25 pairs surviving EG.33's FDR correction — answers EG.27's flagged gap, whether severity confounds the env-SDOH link rather than sitting downstream of it. Track: primary. | **11/25 pairs hold up once severity is controlled for** — these are real environment-SDOH links, not just both tracking severity in parallel. All 4 PM2.5 links survive (food_insecurity_score p=4.1e-16, current_smoker p=2.2e-12, cesd_total p=1.5e-05, paid_total p=1.3e-04), plus VOC/temp/humidity links to food_insecurity_score, substance_use_count, neighborhood_score, current_smoker. The 14 that drop out are mostly the `mean_light` pairs and the weaker single-pollutant hits — severity was doing most of the apparent work there. | keep — this is now the authoritative, doubly-checked (FDR + severity-adjusted) short list of real env-SDOH findings |
| EG.35 | done | `results/EG_35_summary.csv` | Project-head-requested (2026-09-06): does PM2.5 and/or BMI link to blood sugar (CGM or HbA1c), globally or after stratifying by age/diabetic status? First direct age-band/severity-group stratification of this link — all prior stratification (EG.8/24/26) was by clinical site only, which EG.24 showed confounds age/severity composition. Net-of-each-other OLS (PM2.5 and BMI each controlling for the other, + age + site) on hba1c/glucose_cv/tar_180/interday_glucose_variance. Track: primary. | **Yes, both link to blood sugar globally — 8/8 significant.** PM2.5 and BMI are each independently significant for all 4 outcomes pooled (PM2.5 hba1c p=2.0e-06, interday_glucose_variance p=1.1e-05; BMI hba1c p=1.8e-14, tar_180 p=4.5e-09). Age-stratified: the link is strong and consistent under 70 (16/24 significant across the 40-54 and 55-69 bands) but **PM2.5's link disappears entirely at 70+** (0/4 significant) while BMI's persists (3/4 at 70+). Severity-stratified: much weaker overall (5/32) once split 4 ways — PM2.5 significant for Oral Med/hba1c and Insulin/interday_glucose_variance; BMI significant for Pre-DM/hba1c, Pre-DM/tar_180, Oral Med/glucose_cv. | keep — direct, current answer to his question: the global link is real and robust, and it's age (not severity group) where it clearly splits — PM2.5's effect is concentrated under 70 |
| EG.36 | done | `results/EG_36_summary.csv` | Project-head-requested (2026-09-06): rerun EG.35's severity split with only 2 groups (non-diabetic = Healthy+Pre-DM, N=1336; diabetic-treated = Oral Med+Insulin, N=944) instead of 4, to test his hypothesis that the 4-way split (5/32 significant) was underpowered rather than a real absence of effect. Track: primary. | **Confirmed, and the pattern is striking.** 7/16 significant (vs. 5/32 for the 4-way split) — more signal per test. **PM2.5 is significant for ALL 4 outcomes in the diabetic-treated group (hba1c p=0.0002, glucose_cv p=0.0073, tar_180 p=0.011, interday_glucose_variance p=0.0001) and for NONE of the 4 in the non-diabetic group** (all p>0.25). BMI shows the reverse-ish pattern: significant for hba1c/tar_180 in the non-diabetic group, only hba1c in the diabetic-treated group. | keep — cleanest, most specific finding yet: PM2.5's link to blood sugar is concentrated entirely in people already being treated for diabetes |
| EG.37 | done | `results/EG_37_summary.csv` | Project-head-requested (2026-09-10): 3-group version of EG.36 — Healthy / Pre-DM / T2DM (Oral Med+Insulin combined) — because reviewers may object to merging Healthy and Pre-DM but combining the two T2DM arms is defensible. Track: primary. | **The reviewer-safe split preserves the EG.36 result cleanly.** PM2.5 → blood sugar: 4/4 outcomes significant in T2DM (hba1c p=1.5e-04, glucose_cv p=0.0074, tar_180 p=0.011, interday_glucose_variance p=5.3e-05), 0/4 in Healthy, 0/4 in Pre-DM (Pre-DM coefficients actually negative — no hint of an effect). BMI → blood sugar shows up in Pre-DM (hba1c p=0.0076, tar_180 p=0.044) and T2DM (hba1c p=0.0077), not Healthy. | keep — confirms the PM2.5-in-T2DM finding survives the more conservative grouping; this is the version to report |
| EG.38 | done | `results/EG_38_summary.csv` | Project-head-requested (2026-09-10): does the PM2.5→blood-sugar link (T2DM group) or the BMI→blood-sugar link (non-diabetic group) run through reduced physical activity? Activity (steps, active_calories, active_minutes_v1/v2_per_day) ~ predictor + other predictor + age + site, within each 2-group stratum. Single-link check, not formal mediation. Track: primary. | **Reduced activity does NOT explain the PM2.5→blood-sugar link.** In the T2DM group, higher PM2.5 predicts MORE activity, not less (steps p=0.001, active_calories p=0.0002, active_minutes_v1 p=0.018 — all positive) — consistent with the long-standing backwards-direction finding (EG.2/EG.3/EG.26) that denser, more-polluted areas have more walking. So PM2.5's effect on blood sugar in T2DM is not mediated by activity; if anything activity works against it. BMI→activity in the non-diabetic group: only 1/4 significant (active_minutes_v2, walking+running only, p=4.9e-05, negative) — higher BMI relates to less vigorous activity but not to fewer steps or calories. | keep — rules out the activity mechanism for PM2.5; partial/weak support for BMI→less vigorous activity |
| EG.39 | done | `results/EG_39_summary.csv` | Project-head-requested (2026-09-21) summary: all associations between glycemia (hba1c, glucose_mean, tir) and pollutants (PM2.5/NOx/VOC), and both of those with BMI/steps/active_minutes_v1_per_day (anything above sedentary), using EG.37's 3-group split. Simple design (predictor + age + site, no cross-predictor control — distinct from EG.35/37). 27 pairs × 4 strata (pooled + 3 groups). Track: primary. | **Confirms and extends EG.36/37's T2DM-concentration pattern across pollutants and outcomes not tested there.** Block A (glycemia×pollutants): 4/27 significant — all PM2.5, 0 in Healthy, 1 in Pre-DM (glucose_mean, negative direction), 3 in T2DM (hba1c/glucose_mean/tir); NOx and VOC show no significant glycemia link in any group. Block B (glycemia×BMI/steps/active-minutes): 9/27 — BMI→hba1c significant in all 3 groups; steps/active-minutes hit sporadically, mostly in T2DM. Block C (pollutants×BMI/steps/active-minutes): 9/27 — PM2.5→BMI significant in all 3 groups (T2DM weakest, p=0.017); PM2.5→steps significant in Healthy and T2DM but not Pre-DM; NOx/VOC mostly null except VOC→BMI (Pre-DM, T2DM). | keep — comprehensive reference table for the write-up; PM2.5 is the only pollutant with a real glycemic signal, concentrated in T2DM as established |
| EG.40 | done | `results/EG_40_fdr_corrected.csv` (+ `EG_40_hba1c_vs_cgm.csv`) | Project-head-requested (2026-09-22), 3 questions on EG.39: (1) FDR correction? (2) does "PM2.5 links to blood sugar" include HbA1c specifically? (3) BMI/PM2.5 vs HbA1c compared to CGM-derived metrics (glucose_mean, tir) — any HbA1c-only or CGM-only cases? Part 1: Benjamini-Hochberg across EG.39's 108 fits. Parts 2-3: reshape EG.39's existing numbers, no new fits. Track: primary. | **(1)** 23/108 survive FDR at q<0.05 (vs 38/108 raw) — all 3 T2DM PM2.5-glycemia hits survive (hba1c q=0.0006, tir q=0.017) except glucose_mean (q=0.10, does NOT survive); pooled PM2.5/hba1c survives (q<0.0001). **(2)** Yes — PM2.5→hba1c is significant pooled and in T2DM, both surviving FDR; not significant in Healthy or Pre-DM. **(3)** Two HbA1c-only asymmetries found, one FDR-solid and one marginal: **in T2DM, BMI→HbA1c is significant and survives FDR (p=0.0026, q=0.017) with NO matching CGM-metric link** (glucose_mean/tir both p>0.13) — a real, FDR-confirmed divergence. In Healthy, BMI→HbA1c is raw-significant (p=0.033) but does NOT survive FDR (q=0.10), and again has no CGM match — flagged as suggestive, not confirmed. One reverse (CGM-only) case: PM2.5→glucose_mean significant in Pre-DM (p=0.028) with no matching HbA1c link, also raw-only (q=0.094, doesn't survive FDR). | keep — the T2DM BMI/HbA1c-vs-CGM divergence is the one asymmetry that survives FDR and is worth flagging explicitly in the write-up; the other two are raw-only, note the caveat |
...
### EG.33 — 2026-09-04
**Method:** Benjamini-Hochberg FDR correction (statsmodels multipletests) applied across the combined 42 env-SDOH Spearman tests: EG.27's 36 (6 env metrics x 6 original SDOH scores) + EG.32's 6 new food_insecurity_score tests. Reports q-values at q<0.05 and q<0.10, closing the gap EG.27/EG.32 both flagged (uncorrected multiple comparisons). Track: primary.
**Result:** Benjamini-Hochberg FDR correction across the combined 42 env-SDOH tests (EG.27's 36 + EG.32's 6 new food-insecurity tests). Raw p<0.05: 26/42. Survive FDR at q<0.05: 25/42: mean_pm25/food_insecurity_score (rho=0.260, q=4.9e-34), mean_pm25/current_smoker (rho=0.149, q=3.61e-11), mean_light/cesd_total (rho=-0.139, q=7.21e-10), mean_pm25/cesd_total (rho=0.137, q=8.13e-10), mean_light/food_insecurity_score (rho=-0.135, q=1.44e-09), mean_pm25/paid_total (rho=0.133, q=3.35e-09), mean_temp/food_insecurity_score (rho=0.116, q=2.44e-07), mean_temp/substance_use_count (rho=-0.115, q=2.81e-07), mean_voc/food_insecurity_score (rho=0.110, q=1.04e-06), mean_light/paid_total (rho=-0.098, q=2.1e-05), mean_nox/neighborhood_score (rho=0.093, q=7.12e-05), mean_light/substance_use_count (rho=0.080, q=0.00053), mean_nox/substance_use_count (rho=0.075, q=0.0012), mean_light/neighborhood_score (rho=0.075, q=0.00159), mean_hum/paid_total (rho=0.073, q=0.0017), mean_light/diet_score (rho=-0.123, q=0.00282), mean_voc/cesd_total (rho=0.067, q=0.00372), mean_pm25/diet_score (rho=0.118, q=0.004), mean_nox/cesd_total (rho=-0.064, q=0.00571), mean_voc/substance_use_count (rho=0.063, q=0.00571), mean_temp/neighborhood_score (rho=-0.063, q=0.00754), mean_voc/current_smoker (rho=0.056, q=0.0154), mean_hum/food_insecurity_score (rho=0.054, q=0.0192), mean_temp/current_smoker (rho=0.053, q=0.0217), mean_nox/food_insecurity_score (rho=-0.050, q=0.0324). Survive only at q<0.10: 3/42. 1 of the 26 raw-significant hits do NOT survive correction -- these were the weaker EG.27 hits (|rho| mostly 0.03-0.15); the food-insecurity links (rho up to 0.26) all survive, confirming EG.32's read that food insecurity is the strongest, most robust env-SDOH signal in this series, not an artifact of testing 42 things at once.
**Decision:** keep
**Output:** results/EG_33.csv

### EG.34 — 2026-09-04
**Method:** For every env-SDOH pair surviving EG.33's FDR correction (q<0.05), OLS refit of sdoh_score ~ env_metric under 3 covariate sets: raw (env_metric only), age+site adjusted, and severity-adjusted (age + site + severity-group dummies added). Answers EG.27's flagged gap -- whether severity group confounds the environment-SDOH link, not just co-occurs with it. Track: primary.
**Result:** Severity-adjusted OLS refit (sdoh_score ~ env_metric + age + site + severity-group dummies) for all 25 env-SDOH pairs that survived EG.33's FDR correction at q<0.05. Still significant with severity controlled: 11/25: mean_pm25/food_insecurity_score (p=4.1e-16), mean_pm25/current_smoker (p=2.18e-12), mean_pm25/cesd_total (p=1.52e-05), mean_pm25/paid_total (p=0.000128), mean_temp/food_insecurity_score (p=0.00142), mean_temp/substance_use_count (p=0.0101), mean_voc/food_insecurity_score (p=0.0047), mean_hum/paid_total (p=0.0345), mean_voc/substance_use_count (p=0.00377), mean_temp/neighborhood_score (p=0.0112), mean_voc/current_smoker (p=0.00415). Drop out once severity is controlled: 14/25: mean_light/cesd_total (p=0.351), mean_light/food_insecurity_score (p=0.755), mean_light/paid_total (p=0.098), mean_nox/neighborhood_score (p=0.0569), mean_light/substance_use_count (p=0.201), mean_nox/substance_use_count (p=0.97), mean_light/neighborhood_score (p=0.913), mean_light/diet_score (p=0.534), mean_voc/cesd_total (p=0.0966), mean_pm25/diet_score (p=0.969), mean_nox/cesd_total (p=0.181), mean_hum/food_insecurity_score (p=0.169), mean_temp/current_smoker (p=0.152), mean_nox/food_insecurity_score (p=0.213). The pairs that survive are environment-SDOH links independent of severity group, not just both tracking severity in parallel -- the strongest candidates for a real finding.
**Decision:** keep
**Output:** results/EG_34.csv

### EG.35 — 2026-09-06
**Method:** For PM2.5 and BMI each (net of the other, + age + site dummies), OLS on 4 glycemic outcomes (hba1c, glucose_cv, tar_180, interday_glucose_variance -- EG.15's strongest CGM-variability finding): globally (pooled), stratified by 3 age bands (EG.23's 40-54/55-69/70+ convention), and stratified by the 4 severity groups. First age/severity-direct stratification of this link -- prior stratification (EG.8/24/26) was by clinical site only, which EG.24 showed confounds age and severity composition. Track: primary.
**Result:** Global (pooled, net of the other predictor + age + site): 8/8 significant: all/log_pm25/hba1c (p=1.99e-06), all/log_pm25/glucose_cv (p=0.00111), all/log_pm25/tar_180 (p=0.002), all/log_pm25/interday_glucose_variance (p=1.08e-05), all/bmi/hba1c (p=1.83e-14), all/bmi/glucose_cv (p=0.0407), all/bmi/tar_180 (p=4.47e-09), all/bmi/interday_glucose_variance (p=0.000336). Age-band-stratified (40-54/55-69/70+, site as covariate): 16/24 significant: 40-54/log_pm25/hba1c (p=0.00592), 40-54/log_pm25/glucose_cv (p=0.00494), 40-54/log_pm25/interday_glucose_variance (p=0.000943), 40-54/bmi/hba1c (p=1.87e-07), 40-54/bmi/tar_180 (p=4.32e-05), 55-69/log_pm25/hba1c (p=0.00255), 55-69/log_pm25/glucose_cv (p=0.0469), 55-69/log_pm25/tar_180 (p=0.0378), 55-69/log_pm25/interday_glucose_variance (p=0.00108), 55-69/bmi/hba1c (p=0.00117), 55-69/bmi/glucose_cv (p=0.00397), 55-69/bmi/tar_180 (p=0.00347), 55-69/bmi/interday_glucose_variance (p=0.000833), 70+/bmi/hba1c (p=0.000103), 70+/bmi/tar_180 (p=0.0375), 70+/bmi/interday_glucose_variance (p=0.0319). Severity-group-stratified (age + site as covariates): 5/32 significant: Pre-DM/bmi/hba1c (p=0.00756), Pre-DM/bmi/tar_180 (p=0.0441), Oral Med/log_pm25/hba1c (p=0.0201), Oral Med/bmi/glucose_cv (p=0.00946), Insulin/log_pm25/interday_glucose_variance (p=0.00278). This is the first time either link has been stratified by age band or severity group directly (all prior stratification was by clinical site, which EG.24 showed is confounded with age/severity composition).
**Decision:** keep
**Output:** results/EG_35.csv

### EG.36 — 2026-09-06
**Method:** Rerun of EG.35's design (PM2.5 and BMI, each net of the other + age + site, across hba1c/glucose_cv/tar_180/interday_glucose_variance), with the severity-group split collapsed from 4 groups to 2 (non-diabetic = Healthy+Pre-DM; diabetic-treated = Oral Med+Insulin), per project-head request to test whether EG.35's weak 4-way severity split was a sample-size artifact. Track: primary.
**Result:** Global (unchanged from EG.35, rerun for a self-contained table): 8/8 significant. 2-group diabetic-status split (non-diabetic = Healthy+Pre-DM, N=1336; diabetic-treated = Oral Med+Insulin, N=944): 7/16 significant: non-diabetic (Healthy+Pre-DM)/bmi/hba1c (p=1.68e-05), non-diabetic (Healthy+Pre-DM)/bmi/tar_180 (p=0.00303), diabetic-treated (Oral Med+Insulin)/log_pm25/hba1c (p=0.000153), diabetic-treated (Oral Med+Insulin)/log_pm25/glucose_cv (p=0.00735), diabetic-treated (Oral Med+Insulin)/log_pm25/tar_180 (p=0.0106), diabetic-treated (Oral Med+Insulin)/log_pm25/interday_glucose_variance (p=5.3e-05), diabetic-treated (Oral Med+Insulin)/bmi/hba1c (p=0.00769). Compares to EG.35's 4-way severity split, which found 5/32 significant. The 2-group split recovers more signal, confirming the 4-way split was underpowered.
**Decision:** keep
**Output:** results/EG_36.csv

### EG.37 — 2026-09-10
**Method:** Rerun of EG.35/EG.36's design (PM2.5 and BMI each net of the other + age + site, across hba1c/glucose_cv/tar_180/interday_glucose_variance) with a 3-group diabetic-status split: Healthy, Pre-DM, and T2DM (Oral Med + Insulin combined). Reviewer-safe version of EG.36 -- keeps Healthy and Pre-DM separate while still combining the two T2DM treatment arms. Track: primary.
**Result:** 3-group split (Healthy N=776, Pre-DM N=560, T2DM=Oral+Insulin N=944). Significant: Pre-DM/bmi/hba1c (p=0.00756), Pre-DM/bmi/tar_180 (p=0.0441), T2DM (Oral Med+Insulin)/log_pm25/hba1c (p=0.000153), T2DM (Oral Med+Insulin)/log_pm25/glucose_cv (p=0.00735), T2DM (Oral Med+Insulin)/log_pm25/tar_180 (p=0.0106), T2DM (Oral Med+Insulin)/log_pm25/interday_glucose_variance (p=5.3e-05), T2DM (Oral Med+Insulin)/bmi/hba1c (p=0.00769). PM2.5 -> blood sugar: 4/4 outcomes in T2DM, 0/4 in Pre-DM, 0/4 in Healthy. Keeping Healthy and Pre-DM separate (the reviewer-safe split) preserves EG.36's result -- the PM2.5 signal is still concentrated in the T2DM group.
**Decision:** keep
**Output:** results/EG_37.csv

### EG.38 — 2026-09-10
**Method:** Within each of the 2 diabetic-status groups (non-diabetic = Healthy+Pre-DM; T2DM = Oral Med+Insulin), OLS of 4 activity measures (steps, active_calories, active_minutes_v1_per_day, active_minutes_v2_per_day) on log_pm25 and on bmi, each net of the other + age + site. Tests the reduced-activity mechanism for EG.36's two group-specific blood-sugar findings (PM2.5->glycemia in T2DM; BMI->glycemia in non-diabetic). Single-link check, not a formal mediation model. Track: primary.
**Result:** Single-link check for the reduced-activity mechanism, within each diabetic-status group (activity ~ predictor + other predictor + age + site). T2DM group, PM2.5 -> activity: 3/4 significant -- steps (coef=+439, MORE activity, p=0.00104), active_calories (coef=+20.8, MORE activity, p=0.000249), active_minutes_v1_per_day (coef=+12.1, MORE activity, p=0.0176). Non-diabetic group, BMI -> activity: 1/4 significant -- active_minutes_v2_per_day (coef=-1.8, less activity, p=4.92e-05). Both predictors were run in both groups (see EG_38_summary.csv) to show group-specificity. Not a formal mediation test -- a bootstrap indirect-effect model is the next step for any link that shows up.
**Decision:** keep
**Output:** results/EG_38.csv

### EG.39 — 2026-09-21
**Method:** Full association grid requested by project head: 3 glycemic outcomes (hba1c, glucose_mean, tir) x 3 pollutants (PM2.5/NOx/VOC, log1p) [Block A]; the same 3 glycemic outcomes x BMI/steps/active_minutes_v1_per_day (anything above sedentary) [Block B]; and the 3 pollutants x BMI/steps/active_minutes_v1_per_day [Block C]. Each pair fit as simple OLS (predictor + age + site dummies, no cross-predictor control -- distinct from EG.35/37's net-of-each-other design). Pooled + EG.37's 3-group split (Healthy / Pre-DM / T2DM=Oral Med+Insulin). 27 pairs x 4 strata = up to 108 fits. Track: primary.
**Result:** Full 27-pair association grid (glycemia x pollutants, glycemia x BMI/steps/active-minutes, pollutants x BMI/steps/active-minutes), each fit as predictor + age + site (no cross-predictor control), pooled and within the 3-group split (Healthy / Pre-DM / T2DM = Oral Med+Insulin). Block A (glycemia vs pollutants): 4/27 significant across the 3 groups (Healthy: 0, Pre-DM: 1, T2DM (Oral Med+Insulin): 3). Block B (glycemia vs BMI/steps/active-minutes): 9/27 significant across the 3 groups (Healthy: 2, Pre-DM: 3, T2DM (Oral Med+Insulin): 4). Block C (pollutants vs BMI/steps/active-minutes): 9/27 significant across the 3 groups (Healthy: 2, Pre-DM: 3, T2DM (Oral Med+Insulin): 4). Full pair-by-pair, group-by-group table in EG_39_summary.csv. Confirms and extends EG.36/37's pattern (PM2.5 signal concentrated in T2DM) across NOx/VOC and the TIR/mean-glucose outcomes not previously tested in the 3-group split.
**Decision:** keep
**Output:** results/EG_39.csv

### EG.40 — 2026-09-22
**Method:** Part 1: Benjamini-Hochberg FDR correction across EG.39's full 108-fit grid. Part 2: restates PM2.5/hba1c specifically from EG.39 Block A. Part 3: reshapes EG.39's PM2.5 and BMI rows into a side-by-side HbA1c-vs-CGM-derived-metric (glucose_mean, tir) comparison per stratum, flagging HbA1c-only / CGM-only asymmetries -- no new model fits, reuses EG.39's numbers with FDR applied. Track: primary.
**Result:** Part 1 (FDR): 23/108 of EG.39's fits survive Benjamini-Hochberg at q<0.05 (vs. 38/108 raw-significant) -- 15 raw hits do not survive, mostly single-group Block B/C hits with p just under 0.05. All 3 T2DM PM2.5-glycemia hits (hba1c/glucose_mean/tir) and the pooled PM2.5 hits survive FDR; the Pre-DM PM2.5/glucose_mean hit does not. Part 2 (PM2.5 and HbA1c specifically): yes -- PM2.5/hba1c is significant pooled (p=4.4e-09) and in T2DM (p=1.0e-04, both survive FDR), not in Healthy or Pre-DM. Part 3 (BMI/PM2.5, HbA1c vs. CGM-derived metrics [glucose_mean, tir]): HbA1c-significant-but-not-CGM cases: BMI/Healthy, BMI/T2DM (Oral Med+Insulin). CGM-significant-but-not-HbA1c cases: PM2.5/Pre-DM. Notably, BMI's link to HbA1c in Healthy is NOT matched by any CGM-metric link there -- HbA1c and CGM diverge specifically in the Healthy group.
**Decision:** keep
**Output:** results/EG_40.csv
